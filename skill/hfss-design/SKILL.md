...
**收工沉淀**:做出一个**自己验证过、性能达标**的设计后(达标判据 = `check_design_targets` 的 `all_pass`,且用户确认要沉淀),录成/更新 `design/<type>.md` 一张卡:没有该卡就复制 `design/_TEMPLATE.md` 新建,并在 `design/INDEX.md` 加指针。
**录什么有优先级**:闭式公式(最好,一套服务任意 f₀/εr)> λ₀ 归一化常数(注明对应 εr)> 报告性能与出处。**某一款的绝对毫米尺寸不入库**——换个频率或 εr 就作废,留着只会误导后来的检索;它只是本次建模的一次性起手值。机理、建模陷阱、调试踩的坑归 §0.2 的 `knowledge/`。
与 §0.2 的分工仍然是:**`design/` 放可缩放的设计数据(公式/常数/性能),`knowledge/` 只放通用经验、不放数据。**文献来源的设计走 `hfss-search` skill,同样**先建模仿真达标再录卡**,不许照抄论文数字入库。
...
> **用户给的是板厂叠层表(StackUp Report,xlsx/csv)?** 那不走本节,转 `hfss-stackup` skill——
> 层多、厚度不可改、必须和总厚对上,且藏着几条不成文的行业约定(D 层占号不占铜、连续同名 PP 是组总厚)。
> 本节是**自己编层叠**时的约定(层少、厚度自己定)。
...
1. **检索起手**:`search_designs`(传 frequency_ghz/polarization/topology/min_gain_dbi)找匹配卡 → `read_design_card` 取正文看闭式公式 / λ 归一化尺寸。
   **没有匹配卡** → 转 **`hfss-search` skill**(检索文献 → 精读提炼 → 产出「结构理解表 + 起手尺寸」交用户确认)拿到起手值后回本步骤第 2 步;无联网/用户不想查文献时,才退回常规建模 + 收工时新建卡。
